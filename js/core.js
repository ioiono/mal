...
function prn() {
  printer.println.apply(
    {},
    Array.prototype.map.call(arguments, function(exp) {
      return printer._pr_str(exp, true);
    })
  );
}
...
function vals(hm) {
  return Object.keys(hm).map(function(k) {
    return hm[k];
  });
}
...
function apply(f) {
  ...
  return f.apply(
    f,
    args.slice(0, args.length - 1).concat(args[args.length - 1])
  );
}
